...
if ($nomEntite1 == -1 && $nomEntite2 == -1) { // il faut au moins choisir deux entités à relier
    ...
    //echo "dezdezdez";
    header('Location: ../../../vue/eleve/modifUneCopieMEA.php?erreur=' . $erreur . '&idCopieEleve=' . $idCopieEleve);
}
    ...
    if ($nomEntite3==-1 && $nomEntite4==-1) { // que deux entités de reliées
        ...
        if (QueryFunctions::nomAssociationNonDispoCopieEleve($bdd, $idCopieEleve, $nomAssociation)) {
...
if ($erreur != 0) {
    ...
    QueryFunctions::insertNouvelleAssociation($bdd, $idMEA, $nomAssociation);
    QueryFunctions::updateDateModifCopieSysdate($bdd, $idCopieEleve);

    if ($totalEntites==2) { // que deux entités de reliées
        QueryFunctions::insertPatte($bdd, $idMEA, $nomAssociation, $nomEntite1, $cardinalite1);
        QueryFunctions::insertPatte($bdd, $idMEA, $nomAssociation, $nomEntite2, $cardinalite2);
    }

    else if ($totalEntites==3) {// 3 entités sélectionnées
        QueryFunctions::insertPatte($bdd, $idMEA, $nomAssociation, $nomEntite1, $cardinalite1);
        QueryFunctions::insertPatte($bdd, $idMEA, $nomAssociation, $nomEntite2, $cardinalite2);
        QueryFunctions::insertPatte($bdd, $idMEA, $nomAssociation, $nomEntite3, $cardinalite3);
    }
        ...
        QueryFunctions::insertPatte($bdd, $idMEA, $nomAssociation, $nomEntite2, $cardinalite2);
        QueryFunctions::insertPatte($bdd, $idMEA, $nomAssociation, $nomEntite3, $cardinalite3);
        QueryFunctions::insertPatte($bdd, $idMEA, $nomAssociation, $nomEntite4, $cardinalite4);
    }
    ...
    header('Location: ../../../vue/eleve/modifUneCopieMEA.php?idCopieEleve=' . $idCopieEleve);
