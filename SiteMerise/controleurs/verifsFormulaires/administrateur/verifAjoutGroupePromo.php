...
$listePromoPossibles = QueryFunctions::getPromotions($bdd,QueryFunctions::getID($_SESSION['login'],$bdd));
$bonnePromo = false;
...
if (!$bonnePromo) {
    ...
    if(!isset($_POST['nomGroupe']) || empty($_POST['nomGroupe'])){
        header('Location: ../../../vue/administrateur/ajoutGroupePromo.php?erreur=1&idPromo=' . $_POST['choixPromo'] . '&origin=' . $origin);
    }
    else if(QueryFunctions::nomGroupeExiste($bdd,$_POST['nomGroupe'],$_POST['choixPromo'])){
        header('Location: ../../../vue/administrateur/ajoutGroupePromo.php?erreur=2&idPromo=' . $_POST['choixPromo'] . '&origin=' . $origin);
    }
    else { // tout est ok
        ...
        $exec2->execute();
        header('Location: ../../../vue/administrateur/ajoutGroupePromo.php?ajoutOK=1&idPromo=' . $_POST['choixPromo'] . '&origin=' . $origin);
